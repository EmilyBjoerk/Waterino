...
Controller::duration Controller::compute(const Controller::time_point& now) const {
  ...
  auto signal_pct = 1.0f + ee_pid_kp * compute_error_pct(now) + ee_pid_ki * ee_pid_si;
  auto pump_duration_ticks = Controller::default_duration.count() * signal_pct;
  return Controller::duration(
      static_cast<Controller::duration::value_type>(pump_duration_ticks + 0.5f));
}
...
float Controller::compute_error_pct(const Controller::time_point& now) const {
  ...
  Controller::duration tgt_period = ee_tgt_period;
...
void Controller::print_stat(Controller::ostream os, const Controller::time_point& now) const {
